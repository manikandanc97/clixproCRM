...
export class DealPipelineService {
  static async getPipeline(tenantId: string, currency = "USD") {
    ...
    const totalValue = openDeals.reduce((total: number, deal) => total + toNumber(deal.value), 0);
    const weightedPipeline = openDeals.reduce((total: number, deal) => {
      const probability = deal.probability || 10; // Default 10% if undefined
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const activeDealsOnDay = deals.filter(l => l.createdAt < dEnd && (!["WON", "LOST"].includes(l.stage) || l.updatedAt >= dEnd)).length;
      
      const dealsUpToDay = deals.filter(l => l.createdAt < dEnd);
      const wonDealsOnDay = dealsUpToDay.filter(l => l.stage === "WON");
      const winRateOnDay = dealsUpToDay.length ? (wonDealsOnDay.length / dealsUpToDay.length) * 100 : 0;
    ...
    const previousOpenDeals = deals.filter(l => l.createdAt < sevenDaysAgo && (!["WON", "LOST"].includes(l.stage) || l.updatedAt >= sevenDaysAgo)).length;
    const previousClosedDeals = deals.filter(l => ["WON", "LOST"].includes(l.stage) && l.updatedAt < sevenDaysAgo);
    ...
    const previousWinRate = previousDeals.length ? (previousWonDeals.length / previousDeals.length) * 100 : 0;

    const items = deals.map((deal) => {
      const stageLabel = deal.stage;
      const probability = deal.probability || 10;
      
      const daysSinceUpdate = Math.floor((new Date().getTime() - new Date(deal.updatedAt).getTime()) / (1000 * 60 * 60 * 24));
      ...
      const expectedCloseDate = deal.expectedCloseDate || new Date(deal.createdAt.getTime() + 30 * 24 * 60 * 60 * 1000);
      ...
      return {
        ...
        followUp: formatRelativeDate(deal.expectedCloseDate, { fallback: "Not scheduled" }),
        ...
        activityCount: [deal.createdAt, deal.updatedAt, deal.expectedCloseDate].filter(Boolean).length,
        isStuck,
    ...
    return {
      stats: [
        ...
        { title: "Weighted Value", value: formatCurrency(weightedPipeline, currency), valueAmount: weightedPipeline },
