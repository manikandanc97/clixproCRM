...
export class AnalyticsService {
  static async getAnalytics(tenantId: string, filter?: string) {
    ...
    const customersBaseWhere: Prisma.CustomerWhereInput = { tenantId, deletedAt: null };

    const currentLeadsWhere: Prisma.LeadWhereInput = { ...leadsBaseWhere, createdAt: { gte: currentStart, lte: now } };
    const previousLeadsWhere: Prisma.LeadWhereInput = { ...leadsBaseWhere, createdAt: { gte: previousStart, lt: currentStart } };
    const currentTasksWhere: Prisma.TaskWhereInput = { ...tasksBaseWhere, createdAt: { gte: currentStart, lte: now } };
    const previousTasksWhere: Prisma.TaskWhereInput = { ...tasksBaseWhere, createdAt: { gte: previousStart, lt: currentStart } };
    const currentCustomersWhere: Prisma.CustomerWhereInput = { ...customersBaseWhere, createdAt: { gte: currentStart, lte: now } };
    const previousCustomersWhere: Prisma.CustomerWhereInput = { ...customersBaseWhere, createdAt: { gte: previousStart, lt: currentStart } };

    // All counts in one parallel batch
    const [
      leadsCount, previousLeadsCount,
      tasksCount, previousTasksCount,
      customersCount, previousCustomersCount,
      stageCounts,
      revenueTargetRecord,
    ] = await Promise.all([
      prisma.lead.count({ where: currentLeadsWhere }),
      prisma.lead.count({ where: previousLeadsWhere }),
      prisma.task.count({ where: currentTasksWhere }),
      prisma.task.count({ where: previousTasksWhere }),
      prisma.customer.count({ where: currentCustomersWhere }),
      prisma.customer.count({ where: previousCustomersWhere }),
      ...
      prisma.revenueTarget.findFirst({ where: { tenantId, isActive: true }, orderBy: { createdAt: "desc" } }),
    ]);

    // Extract stage counts from groupBy result
    ...
    // Real period-over-period change calculation
    function calcChange(current: number, previous: number): { change: string; positive: boolean } {
      if (previous === 0) return { change: current > 0 ? "+100%" : "0%", positive: current >= 0 };
      const pct = ((current - previous) / previous) * 100;
      const sign = pct >= 0 ? "+" : "";
      return { change: `${sign}${pct.toFixed(1)}%`, positive: pct >= 0 };
    }

    // Real 7-day sparkline from DB — all 7 days in parallel
    async function buildSparkline(
      model: "lead" | "task" | "customer",
      baseWhere: Prisma.LeadWhereInput | Prisma.TaskWhereInput | Prisma.CustomerWhereInput
      ...
      const counts = await Promise.all(
        days.map(({ dayStart, dayEnd }) => {
          const where = { ...baseWhere, createdAt: { gte: dayStart, lte: dayEnd } };
          if (model === "lead") return prisma.lead.count({ where: where as Prisma.LeadWhereInput });
          if (model === "task") return prisma.task.count({ where: where as Prisma.TaskWhereInput });
          return prisma.customer.count({ where: where as Prisma.CustomerWhereInput });
        })
      );
    ...
    const [tasksSparkline, leadsSparkline, customersSparkline] = await Promise.all([
    ...
    // Revenue target: use active DB record, distribute monthly
    const annualTarget = revenueTargetRecord ? toNumber(revenueTargetRecord.value) : 0;
    const monthlyTarget = annualTarget > 0 ? Math.round(annualTarget / 12) : 0;
    const revenueOverview = months.map(month => ({ name: month, target: monthlyTarget, revenue: 0 }));
    ...
    // Real conversion stats from DB
    const totalQualified = wonCount + lostCount; // Only qualified (closed) leads
    const winRate = totalQualified > 0 ? Math.round((wonCount / totalQualified) * 100) : 0;
    ...
    return {
      ...
      customerGrowth: [],
      recentActivity: [],
      // All values are real DB aggregations - no hardcoded values
      conversionStats: {
  ...
  static async getAiInsights(tenantId: string) {
    const leads = await prisma.lead.findMany({ where: { tenantId, stage: "NEW", deletedAt: null }, take: 3, orderBy: { createdAt: 'desc' } });
