...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { 
      ...
      createDeal, dealName, dealValue, dealStage, probability, expectedCloseDate, ownerId 
    } = body;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!finalCustomerId && customerName) {
        ...
        // If not found by email, try to find by exactly matching name and company
        if (!finalCustomerId) {
          const existingByName = await tx.customer.findFirst({
             where: { tenantId, name: { equals: customerName, mode: "insensitive" }, company: { equals: companyName, mode: "insensitive" }, deletedAt: null }
          });
          if (existingByName) {
        ...
        if (!finalCustomerId) {
          const newCustomer = await tx.customer.create({
            data: {
              ...
              revenue: createDeal ? (dealValue || lead.value) : 0,
              status: "ACTIVE",
      ...
      // 3. Create Deal ONLY if explicitly requested
      let deal = null;
      if (createDeal) {
      ...
      await tx.timelineEvent.create({
        data: {
          ...
          description: deal ? `Lead converted to Deal: ${deal.name}` : `Lead converted to Customer`,
