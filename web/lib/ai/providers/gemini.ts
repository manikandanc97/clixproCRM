...
export class GeminiProvider implements AIProvider {
  ...
  async stream(messages: UIMessage[], options?: StreamOptions): Promise<AIStreamResult> {
    ...
    const result = streamText({
      ...
      tools: options?.tools as any,
      onError: (err: any) => {
        appendFileSync('stream-error.log', 'STREAM ERROR: ' + (err?.error?.message || err?.message || JSON.stringify(err)) + '\n');
      },
  ...
  async toolCall(messages: UIMessage[], tools: Record<string, unknown>): Promise<{ text: string; toolCalls: unknown[] }> {
    if (!this.googleAi) throw new Error('GeminiProvider not initialized');
