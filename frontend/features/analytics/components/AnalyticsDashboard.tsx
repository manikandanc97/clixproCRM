...
export default function AnalyticsDashboard() {
  const { data, isLoading: loading, refetch } = useAnalytics();
    ...
    <CRMPageContainer>
      <CRMPageHeader
        ...
        actions={[
          ...
          { label: "Refresh", icon: RefreshCw, onClick: () => refetch(), variant: "outline" },
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <RevenueOverviewChart data={data?.revenueOverview} loading={loading} />
          </motion.div>
          <motion.div variants={itemVariants}>
            <PipelineOverviewChart data={data?.pipelineStages} loading={loading} />
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={itemVariants}>
            <TopAgents data={data?.topAgents} loading={loading} />
          </motion.div>
          <motion.div variants={itemVariants}>
            <LeadsGrowthChart data={data?.leadsGrowth} loading={loading} />
          </motion.div>
          <motion.div variants={itemVariants}>
            <RecentActivity data={data?.recentActivity} loading={loading} />
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={itemVariants}>
             <MonthlyPerformanceChart data={data?.revenueOverview} loading={loading} />
          </motion.div>
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <CustomerGrowthChart data={data?.customerGrowth} loading={loading} />
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-8">
           <motion.div variants={itemVariants}>
             <ConversionAnalyticsChart data={data?.conversionStats} loading={loading} />
          </motion.div>
          <motion.div variants={itemVariants} className="lg:col-span-2">
