...
const QuotationPreview = ({ quotation, isOpen, onClose }: QuotationPreviewProps) => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="sm:max-w-xl p-0 bg-slate-50 border-l-slate-200 shadow-2xl overflow-y-auto">
        {/* Premium Header */}
        <div className="bg-white p-8 border-b border-slate-200 sticky top-0 z-20">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <SheetTitle className="text-2xl font-black text-slate-900 tracking-tighter">
                  {quotation.quoteId}
                </SheetTitle>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-slate-400 text-xs font-medium">Created on May 06, 2026</span>
                </div>
              </div>
            </div>
            
            <div className="flex items-center gap-2">
              <Button size="icon" variant="ghost" className="rounded-xl text-slate-400 hover:text-slate-900">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button className="rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold h-12 shadow-lg shadow-slate-200">
              <Send className="w-4 h-4 mr-2" />
              Send to Client
            </Button>
            <Button variant="outline" className="rounded-2xl border-slate-200 h-12 font-bold text-slate-700">
        ...
        <div className="p-8 space-y-8">
          ...
          <motion.div 
            ...
            className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden"
          >
          ...
          {/* AI Reminders */}
          <section className="bg-emerald-50/50 border border-emerald-100 rounded-3xl p-5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-emerald-100 flex items-center justify-center">
                <Clock className="w-5 h-5 text-emerald-600" />
              </div>
              <div>
                <p className="text-sm font-black text-slate-900">Follow-up due tomorrow</p>
                <p className="text-[11px] text-slate-500 font-medium">AI suggests morning call for 15% better conversion</p>
              </div>
            </div>
          ...
          <section className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-lg font-black text-slate-900">{quotation.client}</p>
                <p className="text-sm text-slate-500 font-medium mt-0.5">Corporate Headquarters, NY</p>
              </div>
          ...
          {/* Itemized Breakdown */}
          <section className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Line Items</h4>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={idx} className="flex gap-4 relative">
                  {idx !== 2 && <div className="absolute left-[15px] top-8 bottom-[-24px] w-0.5 bg-slate-100" />}
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center z-10 shrink-0 shadow-sm
                    ${idx === 2 ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                    <activity.icon className="w-3.5 h-3.5" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-900">{activity.title}</p>
                    <p className="text-[11px] text-slate-400 font-medium mt-0.5">{activity.time} • {activity.user}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 pb-8">
            <Button variant="outline" className="rounded-2xl border-slate-200 h-14 font-bold text-slate-700 hover:bg-white shadow-sm transition-all group">
              <Copy className="w-4 h-4 mr-2 text-slate-400 group-hover:text-slate-600" />
              Duplicate
            </Button>
            <Button variant="outline" className="rounded-2xl border-rose-100 h-14 font-bold text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all">
